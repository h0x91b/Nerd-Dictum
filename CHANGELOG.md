...
[2026-08-29] Dev — `SIMULATE_ERROR_RATE=0.5` makes half of all transcriptions fail with a fake 503 instead of calling Gemini, so the failed-recording + retry flow can be exercised without waiting for a real outage. Run it with `bun run dev:mac:flaky` (override the rate by exporting `SIMULATE_ERROR_RATE` first). The roll happens **once per transcription**, not per attempt — a per-attempt roll would square to 25% because of the built-in retry. Off unless the env var is set; main logs the rate at startup and each simulated failure is logged as `[Gemini] Simulating a 503`.
